...
def generate_skill_md(schema: ToolSchema) -> str:
    ...
    sections = [
        ...
        "",
        "Treat intake as a **required gate** before using this skill at the highest quality level. Ask for every missing fact first instead of guessing.",
        "",
        "### Required checklist",
        "",
        "Confirm the target environment, user goal, authorized access level, authentication method, required outputs, validation expectations, sensitive actions, operational constraints, and success criteria before execution.",
        "",
        "### Stop conditions",
        "",
        "Stop and ask follow-up questions instead of proceeding at full fidelity if the prompt is vague, if the environment is unclear, if protected or sensitive flows are in scope without access details, or if success criteria have not been confirmed.",
        "",
        "### Fallback rule",
        "",
        "Use a narrower execution scope only after explicitly telling the user which checklist items are missing and what limitations those gaps impose.",
        "",
        "## Parameters",
        "",
...
def generate_workflow_skill_md(workflow: Workflow, graph: CapabilityGraph) -> str:
    ...
    lines = [
        ...
        "## Highest-Fidelity Intake",
        "",
        "Treat workflow intake as a **mandatory pre-run checklist**. Before running this workflow, ask the user for the fullest available context: target environment, intended business outcome, access level, credentials or login method, critical workflow steps, approval-gated actions, output requirements, operational constraints, and how success should be validated.",
        "",
        "### Stop conditions",
        "",
        "Do not proceed at highest fidelity if the workflow is being inferred from a URL alone, if any protected or sensitive step lacks permission details, if the safe test environment is unknown, or if success criteria have not been confirmed.",
        "",
        "### Fallback rule",
        "",
        "If the user cannot provide the missing information, continue only with a reduced-scope workflow after stating exactly what is missing and what cannot be guaranteed.",
        "",
        "## Steps",
        "",
    ]
